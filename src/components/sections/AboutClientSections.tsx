...
export function StatsStrip() {
  return (
    <section className="py-14 px-6 relative z-10" style={{ borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
        ...
        {STATS.map(({ value, label }, i) => (
          <div key={label}
            ...
            onMouseEnter={e => {
              ...
              el.style.boxShadow = i%2===0 ? '0 8px 30px rgba(220,26,26,0.15)' : '0 8px 30px rgba(37,99,235,0.15)'
            }}
            onMouseLeave={e => {
              const el = e.currentTarget as HTMLElement
  ...
  return (
    <section className="py-24 px-6 relative z-10 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0" style={{background:'linear-gradient(135deg,rgba(4,9,30,0.97),rgba(10,5,25,0.96))'}}/>
      </div>
      <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <div className="flex flex-col gap-5">
            {[
              { emoji:'👁️', label:'Our Vision', color:'#dc1a1a',
        ...
        <div className="relative">
          <div className="relative rounded-3xl overflow-hidden" style={{height:460}}>
            <Image src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=900&q=85"
              alt="Jaman office" fill className="object-cover"/>
            <div className="absolute inset-0" style={{background:'linear-gradient(to bottom,transparent 50%,rgba(4,9,30,0.75))'}}/>
          </div>
          <div className="absolute -bottom-6 -right-4 rounded-2xl p-5 text-center z-10"
            style={{ background:'linear-gradient(135deg,#a81010,#dc1a1a)', boxShadow:'0 20px 50px rgba(220,26,26,0.5)' }}>
...
export function ValuesSection() {
  return (
    <section className="py-20 px-6 relative z-10 overflow-hidden"
      ...
      <div className="absolute inset-0">
        <div style={{ backgroundImage:"url('https://images.unsplash.com/photo-1573164713988-8665fc963095?w=1600&q=40')", backgroundSize:'cover', backgroundPosition:'center', position:'absolute', inset:0, opacity:0.05 }}/>
        <div className="absolute inset-0" style={{background:'linear-gradient(180deg,rgba(4,9,30,0.97),rgba(4,9,30,0.97))'}}/>
      </div>
...
export function TeamSection() {
  return (
    <section className="py-24 px-6 relative z-10 overflow-hidden">
      <div className="absolute inset-0">
        <div style={{ backgroundImage:"url('https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1600&q=40')", backgroundSize:'cover', backgroundPosition:'center top', position:'absolute', inset:0, opacity:0.07 }}/>
        <div className="absolute inset-0" style={{background:'linear-gradient(180deg,rgba(4,9,30,0.96),rgba(4,9,30,0.96))'}}/>
      </div>
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-14">
          ...
          <p className="text-slate-400 text-lg max-w-xl mx-auto">
            Multi-disciplinary specialists bringing holistic solutions to every engagement.
          </p>
        </div>
          ...
          {TEAM_ROLES.map((member, i) => (
            <div key={member.title}
              ...
              onMouseEnter={e => {
                ...
                el.style.boxShadow = i%2===0 ? '0 10px 30px rgba(220,26,26,0.12)' : '0 10px 30px rgba(37,99,235,0.12)'
              }}
              onMouseLeave={e => {
                const el = e.currentTarget as HTMLElement
                el.style.borderColor=var(--border)
              ...
              <div className="w-16 h-16 rounded-full flex items-center justify-center font-black text-sm text-white"
                style={{ background: i%2===0 ? 'linear-gradient(135deg,#a81010,#dc1a1a)' : 'linear-gradient(135deg,#0e2d8a,#2563eb)', boxShadow: i%2===0 ? '0 6px 20px rgba(220,26,26,0.35)' : '0 6px 20px rgba(14,45,138,0.35)' }}>
                {member.initials}
              </div>
              <h3 className="text-white font-bold text-sm">{member.title}</h3>
              <p className="text-[11px] font-bold uppercase tracking-widest" style={{color: i%2===0 ? '#ff4444' : '#60a5fa'}}>{member.role}</p>
...
export function StandardsSection() {
  return (
    <section className="py-16 px-6 relative z-10" style={{ borderTop:'1px solid rgba(255,255,255,0.06)' }}>
          ...
          {STANDARDS.map((s, i) => (
            <div key={s.title}
              ...
              onMouseEnter={e => {
                const el = e.currentTarget as HTMLElement
                el.style.borderColor = i%2===0 ? 'rgba(220,26,26,0.5)' : 'rgba(37,99,235,0.5)'
                el.style.boxShadow = i%2===0 ? '0 0 30px rgba(220,26,26,0.15)' : '0 0 30px rgba(37,99,235,0.15)'
              }}
              onMouseLeave={e => {
                const el = e.currentTarget as HTMLElement
                el.style.borderColor=var(--border)
